...
export default function MatchCard({ match, index = 0, onLike, onPass }) {
  ...
  return (
    <motion.div
      ...
      style={{ opacity: Math.max(0.3, 1 - Math.abs(swipeX) / 150) }}
    >
      <div className={`card bg-gradient-to-br ${tierStyle} p-4 sm:p-5 relative`}>
        <div className="flex items-start gap-3 sm:gap-4">
          {/* Avatar */}
          <Link to={`/profile/${match.user_id}`} className="flex-shrink-0">
            {photo ? (
          ...
          <Link to={`/profile/${match.user_id}`} className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              {match.they_liked && <span className="text-pink-400 text-xs" title="Liked you">💜</span>}
          ...
          <div className="flex flex-col items-end gap-2 flex-shrink-0">
            ...
            <div className="flex gap-1.5">
              {onPass && (
                <button onClick={(e) => { e.preventDefault(); onPass() }}
              ...
              {match.i_liked && (
                <span className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-purple-600/30 border border-purple-500/40 flex items-center justify-center text-purple-300">♥</span>
              )}
            </div>
          </div>
        </div>
