...
export default function MatchCard({ match, index = 0, onLike, onPass }) {
  ...
  const doLike = () => {
    setExiting(true)
    // Delay the actual callback so animation plays first
    setTimeout(() => { if (onLike) onLike() }, 400)
  }
    ...
    <AnimatePresence>
      {!exiting ? (
        <motion.div
          ...
          transition={{ delay: index * 0.05, duration: exiting ? 0.4 : 0.3 }}
          onTouchStart={handleTouchStart}
          onTouchMove={handleTouchMove}
          onTouchEnd={handleTouchEnd}
          className="relative"
        >
          {/* "Liked ♥" overlay that shows during exit */}
          {exiting && (
            <motion.div className="absolute inset-0 z-10 rounded-2xl bg-purple-600/30 flex items-center justify-center"
              initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              <span className="text-purple-300 text-2xl font-bold">Liked ♥</span>
            </motion.div>
          )}
          ...
          <div className={`card bg-gradient-to-br ${tierStyle} p-4 sm:p-5 relative ${match.i_liked ? 'ring-1 ring-purple-500/40' : ''}`}>
            <div className="flex items-start gap-3 sm:gap-4">
              ...
              <Link to={`/profile/${match.user_id}`} className="flex-shrink-0 relative">
                ...
                {/* BUG 3 FIX: Pink heart badge when they already like you */}
                ...
                {/* Gold MATCH badge for mutual likes */}
                {match.they_liked && match.i_liked && (
              ...
              <div className="flex flex-col items-end gap-2 flex-shrink-0">
                ...
                <div className="flex gap-1.5">
                  {onPass && (
                    <button onClick={(e) => { e.preventDefault(); if (onPass) onPass() }}
                  ...
                  {onLike && !match.i_liked && (
                    <button onClick={(e) => { e.preventDefault(); doLike() }}
